...
function signUp(){
  ...
  if(name === '' || e === '' || pass === '' || cp === '')
  {
      msg.innerHTML = `  <p class="e-msg">Error : All the fields are mandatory </p>  `;

      return;
  }
  else if(pass !== cp)
  {
      msg.innerHTML = ` <p class="e-msg">Error: Password Not Matching, Please check!</p>  `;
      return;
  }
  else if(pass.length < 6 || cp.length < 6)
  {
      msg.innerHTML = `  <p class="e-msg">Error: Password must be 6 or more character long!</p>  `;
  }
  else{
    ...
    // timer.innerHTML = `  <p class="time">You will be redirecting in 2 seconds ...</p>   `;
